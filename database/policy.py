...
for policy in policys:
    ...
    tmp_dict['policy_clients'] = os.popen("bpplclients %s -L | awk '{print $2}' | xargs" %policy).read().strip('\n').split()
    policy_info = os.popen("bpplinfo %s  -U | egrep 'Residence:|Volume Pool:|Policy Type:' | awk -F ':' '{print $2}'" %policy).read().strip(' ').split('\n')
    tmp['policy_type'] = policy_info[0].strip()
    tmp['policy_stu'] = policy_info[1].strip()
    tmp['policy_pool'] = policy_info[2].strip()
    ...
    for label in policy_sched_label:
        ...
        if policy_sched_type == "User Backup" or policy_sched_type == "User Archive":
            tmp_dict['policy_sched_frequency'] = "None"
            tmp_dict['policy_sched_retention'] = policy_sched[1].strip()[3:].strip(')')
        else:
            tmp['policy_sched_frequency'] = policy_sched[1].strip()
            tmp['policy_sched_retention'] = policy_sched[2].strip()[3:].strip(')')       
